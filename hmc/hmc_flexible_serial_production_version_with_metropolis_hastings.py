...
def run_mh(logdensity, num_steps, initial_positions, rng_key, num_chains, num_params, warmup_initial_position, num_warmup_steps):
    """
    Metropolis-Hastings random walk sampler using blackjax.
    
    The proposal is an independent isotropic normal distribution in each parameter:
        x_t = x_{t-1} + eps,  eps ~ N(0, sigma^2 * I)

    Args:
        logdensity:          Log density function to sample from.
        num_steps:           Number of MCMC steps per chain.
        initial_positions:   List of length num_chains, each entry a 1D array of shape (num_params,).
        rng_key:             JAX PRNGKey.
        num_chains:          Number of independent chains.
        num_params:          Number of parameters.
        sigma:               Scalar or array of shape (num_params,) giving the std dev of the
                             normal proposal for each parameter.

    Returns:
        chain_samples:       List of arrays of shape (num_steps, num_params), one per chain.
        acceptance_rates:    List of arrays of shape (num_steps,), one per chain.
    """
    # Build a sigma array of shape (num_params,) so each parameter has its own proposal std dev.
    # blackjax.mcmc.random_walk.normal interprets a 1D sigma as independent per-component normals.
    '''if sigma_array_flag:
        sigma_array = jnp.broadcast_to(jnp.asarray(sigma, dtype=float), (num_params,))
    else:
        sigma_array = sigma'''
    ...
    print("warmup time", warmup_end - warmup_start)
    sigma_array = jnp.array(0.8*parameters["inverse_mass_matrix"])

    # Construct the sampler: additive step RW with a zero-mean normal proposal
    mh = blackjax.additive_step_random_walk(
        logdensity,
        blackjax.mcmc.random_walk.normal(sigma_array)
    )
    ...
    for i in range(num_chains):
        initial_state = mh.init(initial_positions[i])
        rng_key, sample_key = jax.random.split(rng_key)
        states, acceptance_rate = inference_loop(sample_key, mh.step, initial_state, num_steps)
...
def mh_toy_model_main():
    ...
    start = time.time()
    chain_samples, acceptance_rates = run_mh(logdensity, num_steps, initial_positions, rng_key, num_chains, num_params, warmup_initial_position, num_warmup_steps)
    end = time.time()
    print("MH toy time: ", end - start)
    ...
    plt.savefig("metropolis_hastings_trace_plots.png")
    plt.close()
    ...
    np.savez("toy_mcmc_mh_chains.npz", mean1 = mean1_chains, mean2 = mean2_chains)
    ...
    ax.set_title("Toy MCMC - Metropolis Hastings")
    plt.savefig("toy_mcmc_mh.png")
    plt.show()
...
def unlensed_cmb_main_mh(seed, data, true_h0, true_ombh2, true_omch2, noise_level):
    rng_key = jax.random.key(0)
    npix = 64
    ...
    #mh_sigmas = np.array([0.5*40.0, 0.6, 0.5*0.01])
    #mh_sigmas *= 0.01
    '''mh_sigmas = jnp.array([[ 6.1414944e+01,  2.2738088e-02, -8.6694717e-02],
    [ 2.2738090e-02,  1.9587389e-05, -2.3278850e-05],
    [-8.6694717e-02, -2.3278852e-05,  1.6289820e-04]])
    sigma_array_flag = False'''
    rng = np.random.default_rng()
    initial_positions = []
    for i in range(num_chains):
    ...
    start = time.time()
    #num_params, warmup_initial_position, num_warmup_steps
    chain_samples, acceptance_rates = run_mh(logdensity, num_samples, initial_positions, rng_key, num_chains, num_params, warmup_initial_position, num_warmup_steps)
    end = time.time()
    print("mcmc time: ", end - start)

    burn_length = 110000
    num_samples -= burn_length
    

    acceptance_rates_matrix = np.empty((num_chains, num_samples))
    for i in range(num_chains):
        acceptance_rates_matrix[i,:] = acceptance_rates[i][burn_length:]
        print(f"mean acceptance rate for chain {i}: ", np.mean(acceptance_rates[i][burn_length:]))
    np.save(f"mh_acceptance_rates_matrix_seed{seed}.npy", acceptance_rates_matrix)
    ...
    for i in range(num_chains):
        ...
        all_h0_samples.append(h0_samples[burn_length:])
        all_ombh2_samples.append(ombh2_samples[burn_length:])
        all_omch2_samples.append(omch2_samples[burn_length:])
    ...
    np.savez(f"mh_unlensed_cmb_hmc_chains_seed{seed}_gaussianprior.npz", h0_chains = h0_chains, ombh2_chains = ombh2_chains, omch2_chains = omch2_chains)
    ...
    fig_name = f'mh_mcmc_triangle_plot_seed{seed}_gaussianprior.png'
...
def unlensed_cmb_main_nuts(seed, data, true_h0, true_ombh2, true_omch2, noise_level):
    rng_key = jax.random.key(0)
    npix = 64
    ...
    for i in range(num_chains):
        acceptance_rates_matrix[i,:] = acceptance_rates[i][:]
        print(f"mean acceptance rate for chain {i}: ", np.mean(acceptance_rates[i][:]))
    #np.save(f"nuts_acceptance_rates_matrix_seed{seed}.npy", acceptance_rates_matrix)
    ...
    #np.savez(f"nuts_unlensed_cmb_hmc_chains_seed{seed}_gaussianprior.npz", h0_chains = h0_chains, ombh2_chains = ombh2_chains, omch2_chains = omch2_chains)
    ...
    param_names = [r'$H_0$', r'$\Omega_b h^2$', r'$\Omega_c h^2$']
    true_values = [true_h0, true_ombh2, true_omch2]
    #fig_name = f'nuts_mcmc_triangle_plot_seed{seed}_gaussianprior.png'
    fig_name = None
    fig, ax = triangle_plot(final_matrix, param_names, thin_factor = thin_factor, true_values = true_values)
...
def looper():
    ...
    for i in range(1):
        ...
        #plt.show()
        plt.close()
        unlensed_cmb_main_mh(i, map, true_h0, true_ombh2, true_omch2, noise_level) #the map units should be in julia convention here
        #unlensed_cmb_main_nuts(i, map, true_h0, true_ombh2, true_omch2, noise_level) #the map units should be in julia convention here
        print()
