...
def triangle_plot(samples, param_names, bins1d=10, bins2d=10, figsize=None, color='steelblue',
    ...
    gaussian : tuple or None. If provided, should be (mean, cov) where mean is a 1D array
               of shape (n_params,) and cov is a 2D array of shape (n_params, n_params).
               The appropriate marginal Gaussian will be plotted on each pane.
    """
    ...
    for row in range(n_params):
        for col in range(n_params):
            ...
            if col > row:
                ...
                if gaussian is not None:
                    # Marginal 1D Gaussian: just the corresponding mean and variance
                    m = g_mean[row]
                    s = np.sqrt(g_cov[row, row])
                    x = np.linspace(flat[row].min(), flat[row].max(), 300)
                    ax.plot(x, norm.pdf(x, m, s), color='orange', lw=1.5)
                ...
                if gaussian is not None:
                    # Marginal 2D Gaussian: extract sub-mean and sub-covariance
                    idx = [col, row]
                    m2 = g_mean[idx]
                    c2 = g_cov[np.ix_(idx, idx)]
                    # Build grid over the plot range
                    ...
                    ax.contour(X, Y, Z, levels=6, colors='orange', linewidths=1.5)
