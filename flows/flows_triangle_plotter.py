...
def triangle_plot(samples, param_names, bins1d = 20, bins2d = 10, figsize=None, color='steelblue',
    ...
    alpha : float
    true_values : list or None, true parameter values to mark with red vertical lines / stars
    mean_values : list or None, posterior mean values to mark with blue dashed vertical lines
        ...
        for col in range(n_params):
            ...
            if col > row:
                ...
                if mean_values is not None:
                    h = ax.axvline(mean_values[row], color='green', linewidth=1.5,
                                   linestyle='--')
                ...
                # clip_on=False so markers stay visible even at the axes boundary
                if true_values is not None:
                    h, = ax.plot(true_values[col], true_values[row], 'r*',
                                 markersize=10, zorder=5, clip_on=True)
                    legend_handles['Truth'] = h
                if mean_values is not None:
                    h, = ax.plot(mean_values[col], mean_values[row], 'g^',
    ...
    # Shared figure-level legend (top-right corner, outside the grid)
    if legend_handles:
        fig.legend(
